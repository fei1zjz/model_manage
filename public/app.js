...
document.querySelectorAll(".auth-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    document
      ...
      .classList.toggle("hidden", form !== "login");
    document
      .getElementById("register-form")
      .classList.toggle("hidden", form !== "register");
...
async function loadDashboard() {
  try {
    const [servers, allocs, alerts, clusters, health] =
    ...
    document.getElementById("stat-servers").textContent =
      servers.status === "fulfilled" ? servers.value.length : "-";
    document.getElementById("stat-allocations").textContent =
      allocs.status === "fulfilled"
        ? allocs.value.filter((a) => a.status === "ACTIVE").length
        : "-";
    ...
    document.getElementById("stat-clusters").textContent =
      clusters.status === "fulfilled" ? clusters.value.length : "-";
  ...
  try {
    ...
    const tbody =
      ...
      <tr>
        ...
        .join("") ||
      `<tr class="empty-row"><td colspan="7">暂无服务器</td></tr>`;
...
async function viewGPUs(serverId, serverName) {
  try {
    ...
    const text = gpus
      .map(
        (g) =>
          `GPU ${g.index}: ${g.model} ${g.status} ${Math.round(Number(g.memory) / 1024)}GB`,
      )
      .join("\n");
  ...
  try {
    ...
    const tbody =
      ...
      <tr>
        ...
        .join("") ||
      `<tr class="empty-row"><td colspan="6">暂无分配记录</td></tr>`;
  ...
  try {
    ...
    const tbody =
      ...
      <tr>
        ...
        .join("") || `<tr class="empty-row"><td colspan="6">暂无告警</td></tr>`;
...
document
  .getElementById("alert-status-filter")
  .addEventListener("change", loadAlerts);
document
  .getElementById("alert-severity-filter")
  .addEventListener("change", loadAlerts);
document.getElementById("refresh-alerts").addEventListener("click", loadAlerts);

async function ackAlert(id) {
  const by = currentUser?.userId;
  if (!by) return;
  try {
  ...
  try {
    ...
    const tbody =
      ...
      <tr>
        ...
        .join("") || `<tr class="empty-row"><td colspan="7">暂无集群</td></tr>`;
...
async function loadAccount() {
  ...
  try {
    ...
    document.getElementById("profile-role").textContent = badge(user.role);
    document.getElementById("profile-created").textContent = fmt(
      user.createdAt,
    );
  } catch (err) {
    toast(err.message, "error");
...
document
  ...
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const body = { email: fd.get("email"), username: fd.get("username") };
    try {
...
if (token) {
  try {
    ...
    // Fetch current user profile to get full user data
    api("GET", "/users/me")
      .then((user) => {
        currentUser = {
          userId: user.id,
